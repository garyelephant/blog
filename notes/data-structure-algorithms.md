...
```
(1) `叶子`节点的元素被删除，如果元素个数不达标，修复的方法是[按顺序]:
a) 左旋／右旋：当左右兄弟有多余的元素时。
b) 与兄弟合并，同时夹在兄弟之间的父节点对应的元素也要下降到合并后的节点。

2) 在（1）的b)修复的过程中，可能使父节点也不达标，还需要把父节点当作问题节点，按照（1）的方式修复。

(3) `中间`节点的元素被删除，如果元素个数不达标，修复的方法是[按顺序]: TODO
```
