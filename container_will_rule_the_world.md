...
我在日常工作中遇到的一个有趣的例子是：我所在的部门有人希望使用或为其他人提供大数据技术栈中的某些软件服务，如：Elasticsearch, Kibana, Spark, Kafka。他们就像50年前可怜的码头工人一样。[TODO]

我相信以上4点是容器能够定义未来应用发布的几个原力，现在正值原力的觉醒。

正如集装箱对全球运输业及经济的变革不应仅仅归因于集装箱本身而是全社会系统协同创新一样，Docker也将是全系统的变革，除了拥有运行容器的基础技术工具，还需要构建整个生态系统。如 "Build Once, Run Everywhere"需要依赖应用所有的宿主在基础架构上搭建好docker container的运行环境才可以；而要实现现在容器行业大力宣扬的”大规模应用弹性伸缩“，如果没有大规模的容器和资源管理方案（如Mesos, ）是绝不可能实现的。
